...
contract TapRelayer {
    ILastTap public immutable lastTapInstance; // Use the interface type
    ...
    constructor(address _lastTapAddress) {
        ...
        lastTapInstance = ILastTap(_lastTapAddress);
        owner = msg.sender;
    }
    ...
    function approveLastTap(address _tokenAddress, uint256 _amount) external onlyOwner {
        ...
        // address(lastTapInstance) correctly gets the address of the game contract
        token.approve(address(lastTapInstance), _amount);
        emit Approved(_tokenAddress, address(lastTapInstance), _amount);
    }
